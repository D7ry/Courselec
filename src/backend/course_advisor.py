...
class CourseAdvisor:
    ...
    def __init__(self):
        ...
        self.__vector_searcher = VectorSearcher(
            db_uri=db_uri)
        self.__llm = LLM_OpenAI()
        self.__llm_cache_collection = get_mongodb_client(db_uri)[
            "LLM_CACHE"]["OPENAI"]
        pass

    def query(self, prompt: str, school_db_name: str, academic_season_collection_name: str) -> AdvisorResult:
        """
        ...
        if llm_cache is not None:
            ...
            self.__llm_cache_collection.insert_one(
                {"prompt": prompt, "result": nlp_generated_prompt})

        print("NLP generated prompt: " + nlp_generated_prompt)

        print("Searching database...")
        courses = self.__vector_searcher.match(
            db_name=school_db_name,
            collection_name=academic_season_collection_name,
            search_prompt=f"{prompt}, {nlp_generated_prompt}",
            num_return=50)  # get 50 nearest results
